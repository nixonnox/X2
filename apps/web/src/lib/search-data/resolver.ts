...
export async function resolveSearchData(
  ...
  const volumeAdapters = SearchConnectorRegistry.findByCapability("seed_keyword_volume");
  ...
  // 2. 연관 키워드 수집 (병렬)
  const relatedAdapters = SearchConnectorRegistry.findByCapability("related_keywords");
  const autocompleteAdapters = SearchConnectorRegistry.findByCapability("autocomplete");
  const allRelatedAdapters = deduplicateAdapters([...relatedAdapters, ...autocompleteAdapters]);
  ...
  // 3. 트렌드 수집 (병렬)
  const trendAdapters = SearchConnectorRegistry.findByCapability("trend_series");
  ...
  // 4. SERP 수집
  const serpAdapters = SearchConnectorRegistry.findByCapability("serp_documents");
  ...
  // 6. 연관 키워드 중복 제거
  const uniqueRelated = deduplicateRelatedKeywords(relatedKeywords);
...
function deduplicateRelatedKeywords(
  ...
  for (const kw of keywords) {
    const existing = seen.get(kw.normalizedKeyword);
    if (!existing || (kw.avgMonthlySearches ?? 0) > (existing.avgMonthlySearches ?? 0)) {
      seen.set(kw.normalizedKeyword, kw);
    }
  }
  return [...seen.values()];
}
