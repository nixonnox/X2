...
export async function GET(req: NextRequest) {
  ...
  if (!auth) {
    return NextResponse.json(
      { error: "인증이 필요해요. Bearer token을 확인해 주세요.", code: "UNAUTHORIZED" },
      { status: 401 },
    );
  }
  ...
  if (!auth.canAccessApi) {
    return NextResponse.json(
      { error: "API 접근 권한이 없는 플랜이에요. 업그레이드하면 사용할 수 있어요.", code: "PLAN_RESTRICTED" },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    if (action === "mentions") {
      ...
      const mentions = await db.rawSocialMention.findMany({
        ...
        take: limit,
        select: {
          platform: true, text: true, authorName: true,
          publishedAt: true, sentiment: true, postUrl: true,
          viewCount: true, likeCount: true, commentCount: true,
        },
      });
      return NextResponse.json({
        keyword, days, action: "mentions",
    ...
    if (action === "sentiment") {
      const mentions = await db.rawSocialMention.findMany({
        where: { matchedKeyword: keyword, publishedAt: { gte: startDate }, sentiment: { not: null } },
      ...
      return NextResponse.json({
        keyword, days, action: "sentiment",
        data: { total: mentions.length, positive: pos, negative: neg, neutral: neu },
      });
    }
    ...
    return NextResponse.json(
      { error: "데이터를 가져오는 중에 문제가 있었어요.", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
