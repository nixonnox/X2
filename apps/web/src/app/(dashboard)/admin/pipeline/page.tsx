...
function StatusBadge({ ok, label }: { ok: boolean; label: string }) {
  return (
    ...
    >
      ...
      <span
        className={`text-sm font-medium ${ok ? "text-emerald-700" : "text-red-700"}`}
      >
        {label}
      </span>
    </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <StatusBadge
                ...
                label={`PostgreSQL (${status.database.tables}개 테이블)`}
              />
              <StatusBadge
                ok={status.redis.connected}
                label={`Redis ${status.redis.version}`}
              />
              <StatusBadge
                ok={status.worker.running}
                label={status.worker.running ? "워커 실행 중" : "워커 중지"}
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="rounded-xl border border-gray-200 bg-white p-4 text-center">
                <Database className="mx-auto mb-1 h-5 w-5 text-gray-400" />
                <p className="text-2xl font-bold text-gray-900">
                  {status.keywords.saved}
                </p>
                <p className="text-[11px] text-gray-500">저장된 키워드</p>
              </div>
              <div className="rounded-xl border border-gray-200 bg-white p-4 text-center">
                <Server className="mx-auto mb-1 h-5 w-5 text-gray-400" />
                <p className="text-2xl font-bold text-gray-900">
                  {status.mentions.total}
                </p>
              ...
              <div className="rounded-xl border border-gray-200 bg-white p-4 text-center">
                ...
                <p className="text-2xl font-bold text-indigo-600">
                  {status.mentions.today}
                </p>
                <p className="text-[11px] text-gray-500">오늘 수집</p>
              </div>
              <div className="rounded-xl border border-gray-200 bg-white p-4 text-center">
                <Activity className="mx-auto mb-1 h-5 w-5 text-gray-400" />
                <p className="text-2xl font-bold text-indigo-600">
                  {status.snapshots.today}
                </p>
          ...
          {(!status.redis.connected ||
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <ul className="space-y-1 text-[12px] text-amber-600">
                {!status.redis.connected && (
                  <li>Redis 5.0+ 필요 (현재 미연결 또는 버전 부족)</li>
                )}
                {!status.worker.running && (
                  <li>
                    워커가 실행되지 않고 있습니다. `pnpm --filter @x2/analyzer
                    dev` 실행 필요
                  </li>
                )}
                {status.keywords.saved === 0 && (
                  <li>
                    저장된 키워드가 없습니다. 인텔리전스 허브에서 키워드를
                    북마크하세요
                  </li>
                )}
              </ul>
