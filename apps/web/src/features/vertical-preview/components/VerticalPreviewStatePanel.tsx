...
export function VerticalPreviewStatePanel({ screenState }: Props) {
  ...
  if (
    ...
    screenState.qualityWarnings.length > 0
  ) {
    return (
          ...
          <div className="space-y-1">
            {screenState.isMockBased && (
              <p className="text-sm text-amber-700">
                모의 데이터 기반 — 실제 검색 결과와 차이가 있을 수 있습니다.
              </p>
            )}
            {screenState.isStale && (
              <p className="text-sm text-amber-700">
                오래된 데이터 — 최신 데이터로 재검색을 권장합니다.
              </p>
            )}
            {screenState.isPartial && (
              <p className="text-sm text-amber-700">
                부분 데이터 — 일부 소스의 데이터가 누락되었습니다.
              </p>
            )}
          </div>
        </div>
  ...
  if (screenState.status === "idle") {
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
        <h3 className="mb-2 font-medium text-gray-700">업종별 비교 프리뷰</h3>
        <p className="text-sm text-gray-500">
          키워드와 문서 유형을 선택하고 비교를 시작하세요.
          <br />
          같은 데이터로 뷰티 / F&B / 금융 / 엔터테인먼트 4개 업종의
          <br />
          결과 차이를 비교할 수 있습니다.
        </p>
      </div>
